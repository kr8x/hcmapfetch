...
Fetch and demux HamClock VOACAP propagation area maps from the a 
...
Two server protocols are supported:

| Version | Protocol | Detection |
|---|---|---|
| v4.xx+ | Two zlib-compressed streams | `X-2Z-lengths` HTTP header present |
| v3.xx  | Two raw BMP files concatenated | No `X-2Z-lengths` header; use `-V 3.10` |
...
```bash
unzip hcmapfetch.zip
cd hcmapfetch
make
```

This builds `hcmapdemux` against the bundled `zlib-hc` library.
...
Connection:
  ...
  -V VERSION    Protocol version: 4 (zlib, default) or 3.10 (raw BMP)
...
Output:
  ...
  -b BINARY     Path to hcmapdemux          (default: ./hcmapdemux)
